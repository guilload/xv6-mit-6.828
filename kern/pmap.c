...
mem_init(void)
{
	...
	// Your code goes here:
	uint32_t psize = ROUNDUP(npages * sizeof(struct PageInfo), PGSIZE);
	boot_map_region(kern_pgdir, UPAGES, psize, PADDR(pages), PTE_U);
	...
	// LAB 3: Your code here.
	uint32_t esize = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
	boot_map_region(kern_pgdir, UENVS, esize, PADDR(envs), PTE_U);
...
page_init(void)
{
	...
	uint32_t nextfree = (uint32_t) PADDR(boot_alloc(0));
	...
	for (i = 1; i < npages; i++) {
		...
		if (offset < npages_basemem * PGSIZE || (offset >= nextfree && offset < KERNBASE)) {
			pages[i].pp_ref = 0;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if((pt_entry = pgdir_walk(pgdir, va, false)) == NULL || !(*pt_entry & PTE_P)) return NULL;
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if((page = page_lookup(pgdir, va, &pt_entry)) == NULL) return;

	page_decref(page); // will free the page if necessary
	*pt_entry = (pte_t) 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (uintptr_t i = start; i < end; i += PGSIZE) {
		if (i > ULIM ||
			(pt_entry = pgdir_walk(env->env_pgdir, (void *) i, false)) == NULL ||
			(*pt_entry & perm) != perm) {
			user_mem_check_addr = (address > i) ? address : i;
			return -E_FAULT;
		}
	}
